...
import { BuyerNode, EngineNode, VaultNode, ProviderNode, EvaluatorNode, SettlementNode } from '../nodes/CustomNodes'
...
const PHASE_NODES = {
  ...
  submitted: ['buyer', 'engine', 'vault', 'provider', 'evaluator'],
  evaluated: ['buyer', 'engine', 'vault', 'provider', 'evaluator'],
  settling: ['buyer', 'engine', 'vault', 'provider', 'evaluator', 'settlement'],
  slashed: ['buyer', 'engine', 'vault', 'provider', 'evaluator'],
  settled: ['buyer', 'engine', 'vault', 'provider', 'evaluator', 'settlement'],
}

const PHASE_ORDER = ['idle', 'pending_approval', 'published', 'claimed', 'submitted', 'evaluated', 'settling', 'slashed', 'settled']
...
function buildEdges(phase) {
  ...
  return [
    ...
    { id: 'e3', source: 'provider', target: 'engine', label: 'Claim/Submit', animated: phase === 'claimed' || phase === 'submitted' || phase === 'evaluated', markerEnd: { type: MarkerType.ArrowClosed }, style: { stroke: colors['provider->engine'] || colors.stroke, strokeWidth: 2 } },
    { id: 'e4', source: 'engine', target: 'evaluator', label: 'Dual-Track', animated: phase === 'submitted' || phase === 'evaluated', markerEnd: { type: MarkerType.ArrowClosed }, style: { stroke: colors['engine->evaluator'] || colors.stroke, strokeWidth: 2 } },
